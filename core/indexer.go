...
// 反向索引表的一行，收集了一个搜索键出现的所有文档，按照 DocID 从小到大排序。
...
func (indexer *Indexer) Init(options types.IndexerInitOptions) {
	if indexer.initialized == true {
		log.Fatal("The indexer cannot be initialized twice")
	}
	options.Init()
...
func (indexer *Indexer) AddDocumentToCache(document *types.DocumentIndex, forceUpdate bool) {
	if indexer.initialized == false {
		log.Fatal("Indexer has not been initialized")
	}
		...
		if found {
			...
			if indexer.initOptions.IndexType == types.LocationsIndex {
				...
				if numTokensWithLocations != len(tokens) {
					...
					numDocs++
					//当某个关键字对应多个文档且有 lable 关键字存在时，若直接 break,将会丢失相当一部分搜索结果
					continue
				}
			...
			// 当为 LocationsIndex 或者 FrequenciesIndex 时计算 BM25
			if indexer.initOptions.IndexType == types.LocationsIndex ||
